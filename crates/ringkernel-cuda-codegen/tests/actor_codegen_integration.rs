...
fn test_envelope_format_generates_message_header() {
    ...
    assert!(cuda_code.contains("MessageHeader"),
            "Should generate MessageHeader struct");
    assert!(cuda_code.contains("MESSAGE_MAGIC"),
            "Should include message magic constant");
    assert!(cuda_code.contains("MESSAGE_HEADER_SIZE"),
            "Should define header size");
    assert!(cuda_code.contains("HlcTimestamp"),
            "Header should include HlcTimestamp");
    assert!(cuda_code.contains("correlation_id"),
...
fn test_envelope_format_generates_payload_access() {
    ...
    let config = RingKernelConfig::new("payload_test")
        .with_envelope_format(true);
    ...
    assert!(cuda_code.contains("message_get_header(envelope_ptr)"),
            "Should parse message header");
    assert!(cuda_code.contains("message_get_payload(envelope_ptr)"),
            "Should get payload pointer");
    assert!(cuda_code.contains("message_header_validate"),
...
fn test_envelope_hlc_update_from_message() {
    ...
    assert!(cuda_code.contains("Update HLC from message timestamp"),
            "Should update HLC from incoming message");
    assert!(cuda_code.contains("msg_header->timestamp.physical"),
            "Should read message timestamp");
    assert!(cuda_code.contains("hlc_physical = msg_header->timestamp.physical"),
...
fn test_envelope_response_serialization() {
    ...
    assert!(cuda_code.contains("Response serialization (envelope format)"),
            "Should generate envelope response serialization");
    assert!(cuda_code.contains("message_create_response_header"),
            "Should create response header");
    assert!(cuda_code.contains("resp_header, msg_header, KERNEL_ID"),
            "Should pass correlation data");
    assert!(cuda_code.contains("memcpy(resp_envelope + MESSAGE_HEADER_SIZE"),
            "Should copy payload after header");
    assert!(cuda_code.contains("__threadfence()"),
...
fn test_kernel_identity_constants() {
    ...
    assert!(cuda_code.contains("KERNEL_ID = 12345ULL"),
            "Should define kernel ID");
    assert!(cuda_code.contains("HLC_NODE_ID = 99ULL"),
...
fn test_k2k_envelope_format() {
    ...
    assert!(cuda_code.contains("k2k_send_envelope"),
            "Should generate envelope-aware K2K send");
    assert!(cuda_code.contains("k2k_try_recv_envelope"),
            "Should generate envelope-aware K2K receive");
    assert!(cuda_code.contains("k2k_peek_envelope"),
            "Should generate envelope-aware K2K peek");
    assert!(cuda_code.contains("K2KRoutingTable"),
            "Should include K2K routing table");
    assert!(cuda_code.contains("K2KInboxHeader"),
...
fn test_envelope_size_constants() {
    ...
    assert!(cuda_code.contains("PAYLOAD_SIZE = 64"),
            "Should define payload size");
    assert!(cuda_code.contains("MSG_SIZE = MESSAGE_HEADER_SIZE + PAYLOAD_SIZE"),
            "MSG_SIZE should include header");
    assert!(cuda_code.contains("RESP_PAYLOAD_SIZE = 32"),
            "Should define response payload size");
    assert!(cuda_code.contains("RESP_SIZE = MESSAGE_HEADER_SIZE + RESP_PAYLOAD_SIZE"),
...
fn test_raw_format_backwards_compatible() {
    ...
    assert!(cuda_code.contains("MSG_SIZE = 64"),
            "Raw format should use direct message size");
    assert!(cuda_code.contains("msg_ptr = envelope_ptr"),
...
fn test_full_kernel_with_all_features() {
    ...
    assert!(cuda_code.contains("extern \"C\" __global__ void ring_kernel_full_featured"),
            "Should generate correct kernel name");
    assert!(cuda_code.contains("ControlBlock* __restrict__ control"),
            "Should have control block parameter");
    assert!(cuda_code.contains("K2KRoutingTable* __restrict__ k2k_routes"),
            "Should have K2K routes parameter");

    // Verify all struct definitions
    assert!(cuda_code.contains("struct __align__(128) ControlBlock"),
            "Should include ControlBlock");
    assert!(cuda_code.contains("struct HlcState"),
            "Should include HlcState");
    assert!(cuda_code.contains("struct __align__(64) MessageHeader"),
            "Should include MessageHeader");
    assert!(cuda_code.contains("struct K2KRoute"),
            "Should include K2KRoute");

    // Verify message loop structure
    assert!(cuda_code.contains("while (true)"),
            "Should have persistent loop");
    assert!(cuda_code.contains("should_terminate"),
            "Should check termination");
    assert!(cuda_code.contains("is_active"),
            "Should check active state");
    assert!(cuda_code.contains("message_header_validate"),
            "Should validate messages");

    // Verify handler code is included
    assert!(cuda_code.contains("USER HANDLER CODE"),
            "Should include handler section");
    assert!(cuda_code.contains("__syncthreads()"),
...
fn test_helper_functions_generated() {
    ...
    // Message helpers
    assert!(cuda_code.contains("__device__ inline int message_header_validate"),
            "Should have header validation function");
    assert!(cuda_code.contains("__device__ inline unsigned char* message_get_payload"),
            "Should have payload getter");
    assert!(cuda_code.contains("__device__ inline MessageHeader* message_get_header"),
            "Should have header getter");
    assert!(cuda_code.contains("__device__ inline void message_create_response_header"),
            "Should have response header creator");

    // K2K helpers
    assert!(cuda_code.contains("__device__ inline int k2k_send"),
            "Should have k2k_send");
    assert!(cuda_code.contains("__device__ inline int k2k_has_message"),
            "Should have k2k_has_message");
    assert!(cuda_code.contains("__device__ inline unsigned int k2k_pending_count"),
...
fn test_correlation_id_preserved() {
    ...
    let config = RingKernelConfig::new("correlation_test")
        .with_envelope_format(true);
    ...
    assert!(cuda_code.contains("correlation_id = request->correlation_id"),
            "Response should preserve correlation ID");
    assert!(cuda_code.contains("dest_kernel = request->source_kernel"),
...
fn test_message_type_in_generated_code() {
    ...
    let config = RingKernelConfig::new("type_test")
        .with_envelope_format(true);
    ...
    assert!(cuda_code.contains("MyCustomMessage* msg"),
            "Should cast to correct message type");
    assert!(cuda_code.contains("(MyCustomMessage*)msg_ptr"),
