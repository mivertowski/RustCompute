...
pub use validation::{
    is_simple_assignment, validate_function, validate_function_with_mode, validate_stencil_signature,
...
mod tests {
    ...
    fn test_ring_kernel_transpile() {
        ...
        assert!(cuda.contains("struct __align__(128) ControlBlock"), "Should have ControlBlock struct");
        assert!(cuda.contains("is_active"), "Should have is_active field");
        assert!(cuda.contains("should_terminate"), "Should have should_terminate field");
        assert!(cuda.contains("messages_processed"), "Should have messages_processed field");

        // Check kernel signature
        assert!(cuda.contains("extern \"C\" __global__ void ring_kernel_processor"), "Should have correct kernel name");
        assert!(cuda.contains("ControlBlock* __restrict__ control"), "Should have control block param");
        assert!(cuda.contains("input_buffer"), "Should have input buffer param");
        assert!(cuda.contains("output_buffer"), "Should have output buffer param");

        // Check preamble
        assert!(cuda.contains("int tid = threadIdx.x + blockIdx.x * blockDim.x"), "Should have thread id calculation");
        assert!(cuda.contains("MSG_SIZE"), "Should have message size constant");
        assert!(cuda.contains("hlc_physical"), "Should have HLC variables");
        assert!(cuda.contains("hlc_logical"), "Should have HLC logical counter");

        // Check message loop
        assert!(cuda.contains("while (true)"), "Should have persistent loop");
        assert!(cuda.contains("atomicAdd(&control->should_terminate, 0)"), "Should check termination");
        assert!(cuda.contains("atomicAdd(&control->is_active, 0)"), "Should check is_active");

        // Check handler code is embedded
        assert!(cuda.contains("// === USER HANDLER CODE ==="), "Should have handler marker");
        assert!(cuda.contains("value * 2.0"), "Should contain handler logic");
        assert!(cuda.contains("// === END HANDLER CODE ==="), "Should have end marker");

        // Check epilogue
        assert!(cuda.contains("atomicExch(&control->has_terminated, 1)"), "Should mark terminated");
    ...
    fn test_ring_kernel_with_k2k() {
        ...
        // Check K2K structs
        assert!(cuda.contains("K2KRoutingTable"), "Should have K2K routing table");
    ...
    fn test_handler_with_context_methods() {
        ...
        let result = transpile_ring_kernel(&handler, &config);
        assert!(result.is_ok(), "Should transpile handler with context: {:?}", result);

        let cuda = result.unwrap();

        // Context methods should be inlined
        assert!(cuda.contains("threadIdx.x"), "ctx.thread_id() should become threadIdx.x");
        assert!(cuda.contains("__syncthreads()"), "ctx.sync_threads() should become __syncthreads()");
    ...
    fn test_handler_with_message_param() {
        ...
        let result = transpile_ring_kernel(&handler, &config);
        assert!(result.is_ok(), "Should transpile handler with message: {:?}", result);
    ...
    fn test_full_handler_integration() {
        ...
        let result = transpile_ring_kernel(&handler, &config);
        assert!(result.is_ok(), "Should transpile full handler: {:?}", result);
        ...
        assert!(cuda.contains("blockIdx.x * blockDim.x + threadIdx.x"), "Global thread ID");
    ...
    fn test_k2k_handler_integration() {
        ...
        assert!(cuda.contains("K2KRoutingTable"), "Should have K2KRoutingTable");
        assert!(cuda.contains("K2KRoute"), "Should have K2KRoute struct");
        assert!(cuda.contains("K2KInboxHeader"), "Should have K2KInboxHeader");
        assert!(cuda.contains("k2k_routes"), "Should have k2k_routes param");
        assert!(cuda.contains("k2k_inbox"), "Should have k2k_inbox param");
        assert!(cuda.contains("k2k_outbox"), "Should have k2k_outbox param");
        assert!(cuda.contains("k2k_send"), "Should have k2k_send function");
        assert!(cuda.contains("k2k_try_recv"), "Should have k2k_try_recv function");
        assert!(cuda.contains("k2k_peek"), "Should have k2k_peek function");
        assert!(cuda.contains("k2k_pending_count"), "Should have k2k_pending_count function");
    ...
    fn test_all_kernel_types_comparison() {
        ...
        assert!(!stencil_cuda.contains("GridPos"), "Stencil should remove GridPos");
        assert!(stencil_cuda.contains("buffer_width"), "Stencil should have buffer_width");
        ...
        assert!(ring_cuda.contains("ControlBlock"), "Ring kernel ControlBlock");
