...
struct QueueHeader {
    head: u64, tail: u64, capacity: u32, mask: u32, _reserved: [u64; 5],
}

#[repr(C, align(64))]
#[derive(Debug, Clone, Copy, Default)]
struct H2KMessage {
    cmd: u32, flags: u32, cmd_id: u64, param1: u64, param2: u32, param3: u32,
    param4: f32, param5: f32, _reserved: [u64; 3],
}

#[repr(C, align(64))]
#[derive(Debug, Clone, Copy, Default)]
struct K2HMessage {
    resp_type: u32, status: u32, cmd_id: u64, param1: u64, param2: u32, param3: u32,
    param4: f32, param5: f32, _reserved: [u64; 3],
}
...
#[derive(Debug, Clone, Copy, Default)]
struct ActorControl {
    state: u32, role: u32, restart_count: u32, parent_id: u32,
    heartbeat_ns: u64, events_processed: u64, events_forwarded: u64, events_filtered: u64,
    aggregate_sum: f32, aggregate_min: f32, aggregate_max: f32, aggregate_count: u32,
}

#[repr(C)]
#[derive(Debug, Clone, Copy, Default)]
struct PipelineMetrics {
    total_ingested: u64, total_filtered_out: u64, total_aggregated: u64, total_alerts: u64,
    pipeline_latency_ns: u64, throughput_events_per_sec: f32, filter_rate: f32,
    avg_value: f32, active_actors: u32, _pad: [u32; 3],
}
...
struct MappedBuf<T: Copy> {
    host_ptr: *mut T, device_ptr: u64, len: usize,
}
...
impl<T: Copy> MappedBuf<T> {
    fn new(len: usize) -> Self {
        ...
        unsafe {
            let r = cuda_sys::cuMemHostAlloc(&mut host_ptr, size,
...
fn wait_response(hdr: &MappedBuf<QueueHeader>, slots: &MappedBuf<K2HMessage>, rtype: u32, timeout: Duration) -> Option<K2HMessage> {
    let deadline = Instant::now() + timeout;
    while Instant::now() < deadline {
        for r in poll_k2h(hdr, slots) { if r.resp_type == rtype { return Some(r); } }
...
fn test_streaming_pipeline_full() {
    let device = match ringkernel_cuda::CudaDevice::new(0) {
        ...
        _ => { println!("Skipping: need Hopper GPU"); return; }
    };
    ...
    let func = module.get_function("streaming_pipeline_kernel").expect("get function");
    ...
    // Init queue headers
    h2k_hdr.write(0, QueueHeader { head: 0, tail: 0, capacity: QUEUE_CAPACITY as u32, mask: (QUEUE_CAPACITY-1) as u32, _reserved: [0; 5] });
    k2h_hdr.write(0, QueueHeader { head: 0, tail: 0, capacity: QUEUE_CAPACITY as u32, mask: (QUEUE_CAPACITY-1) as u32, _reserved: [0; 5] });
    ...
    let mut ptrs: Vec<u64> = vec![
        h2k_hdr.device_ptr(), h2k_slots.device_ptr(),
        k2h_hdr.device_ptr(), k2h_slots.device_ptr(),
        actor_ctrls.device_ptr(), metrics.device_ptr(),
        inter_bufs_dev, event_buf.device_ptr(), event_count.device_ptr(),
        term_dev,
    ];
    ...
    unsafe {
        cudarc::driver::result::launch_cooperative_kernel(
            func, (num_blocks, 1, 1), (block_size, 1, 1), 0, stream, &mut params,
    ...
    println!("║ Blocks: {}, Threads/block: {}, grid.sync every {} iters         ║",
        num_blocks, block_size, 64);
    println!("║ Filter threshold: {:.1}, Alert threshold: {:.1}                      ║",
    ...
    for &(slot, role, name) in &actors {
        let msg = H2KMessage { cmd: CMD_CREATE_ACTOR, cmd_id, param1: slot as u64, param2: role, ..Default::default() };
        cmd_id += 1;
    ...
    for &count in &inject_sizes {
        ...
        for i in 0..count {
            ...
            event_buf.write(i as usize, Event {
                timestamp: 0,
        ...
        println!("  Injected {:>5} events: {:>10.0} events/s, {:.0}ms pipeline time, \
                  ingested={}, filtered={}, alerts={}",
            count, events_per_sec, latency_ms,
            ingested, m_after.total_filtered_out, m_after.total_alerts);
    ...
    println!("  Total filtered out: {} ({:.1}%)", m.total_filtered_out,
        if m.total_ingested > 0 { m.total_filtered_out as f64 / m.total_ingested as f64 * 100.0 } else { 0.0 });
    ...
    for &(slot, _, name) in &actors {
        ...
        println!("    {:>12}: processed={:>8}, forwarded={:>8}, filtered={:>8}",
    ...
    let msg = H2KMessage { cmd: 17, cmd_id, param1: 4, ..Default::default() }; // Destroy Filter-2
    cmd_id += 1;
    send_h2k(&h2k_hdr, &h2k_slots, msg);
    std::thread::sleep(Duration::from_millis(300));

    let agg_after = actor_ctrls.read(5).events_processed;
    println!("  Aggregate actor continued: {} → {} events (+{})",
        agg_before, agg_after, agg_after - agg_before);

    // ─── Terminate ──────────────────────────────────────────────────
    send_h2k(&h2k_hdr, &h2k_slots, H2KMessage { cmd: CMD_TERMINATE, cmd_id, ..Default::default() });
    ...
    for (count, eps, ms) in &throughput_results {
        println!("║   {:>5} events: {:>10.0} events/s  ({:.0}ms)                        ║", count, eps, ms);
    }
    println!("║                                                                      ║");
    println!("║ Pipeline Totals:                                                     ║");
    println!("║   Ingested:     {:>10}                                              ║", m.total_ingested);
    println!("║   Filtered out: {:>10} ({:.1}%)                                     ║",
        m.total_filtered_out,
        if m.total_ingested > 0 { m.total_filtered_out as f64 / m.total_ingested as f64 * 100.0 } else { 0.0 });
    println!("║   Aggregated:   {:>10}                                              ║", m.total_aggregated);
    println!("║   Alerts:       {:>10}                                              ║", m.total_alerts);
    println!("║   Avg value:    {:>10.4}                                              ║", m.avg_value);
    println!("║                                                                      ║");
    ...
    assert!(m.total_filtered_out > 0, "Filter should have discarded some events");
}
