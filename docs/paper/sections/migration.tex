...
the system into the source's inbox, the in-flight buffer, or the
target's inbox is in exactly one of the following sets:
the source's inbox (for \(\mathit{phase} \in
\{\state{Running}\}\)); the in-flight buffer (for
\(\mathit{phase} \in \{\state{Quiesced}, \state{Transferring},
\state{Swapping}\}\)); the target's inbox (for
\(\mathit{phase} \in \{\state{Swapping}, \state{Complete}\}\) after
\textsc{Transfer}); the delivered set; or, after \textsc{Finalize},
inertia (drained from the source).
...
target-device memory.  Latency is bounded by the NVLink fabric
($\sim$5~$\mu$s for a 4-KiB payload on H100~NVL with NVLink~4).
...
Latency is bounded by host memory bandwidth and the PCIe round
trip ($\sim$25~$\mu$s for the same 4-KiB payload).
\end{itemize}
