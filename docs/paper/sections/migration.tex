...
target-device memory.  Latency is bounded by the NVLink fabric and
is roughly an order of magnitude lower than the host-staged
fallback below; we report \emph{single-GPU} numbers in
\Cref{sec:evaluation} and defer multi-GPU latency measurement to
the future-work program described in \Cref{sec:disc:hw}.
...
round-trip; the dominant cost is the second device-side copy.
\end{itemize}
