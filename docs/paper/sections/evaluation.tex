...
Mean throughput & \textbf{5.54~M~msg/s} \\
Coefficient of variation & \textbf{0.05\%} \\
p50 latency & 70 ns \\
p95 latency & 80 ns \\
p99 latency & 100 ns \\
Throughput drift (first vs.\ last 1-s window) & 0.1\% \\
Memory growth & 0 MiB \\
ECC errors & 0 \\
\bottomrule
\end{tabular}
\caption{K2K sustained run, 60 seconds, 1~KiB payload.  CV of
0.05\% is two orders of magnitude below the conventional
``extremely stable'' threshold (1\%) and three orders below the
``stable'' threshold (5\%).}
\label{tab:k2k-sustained}
\end{table}

The throughput in \Cref{tab:k2k-sustained} is lower than the
inverse of the 70~ns~p50 latency would suggest: the gap is the
...
product of actor count and tick rate.  At 5.54~M~msg/s with a
4-block, 256-thread cluster, this is roughly 5.4~K
