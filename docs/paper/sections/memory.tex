...
small-step traces the actor system can realize.  This section
describes the GPU memory hierarchy as it applies to \system{}, maps
the abstract model components onto physical tiers, and identifies the
constraints on the abstract semantics that the device imposes.
...
\begin{enumerate}
\item \textbf{Producer release}: at the end of \textsc{SendOK}, the
producer issues a \texttt{cuda::atomic\_thread\_fence(\\
...
the consumer issues a matching \texttt{cuda::atomic\_thread\_fence(\\
acquire, scope::cluster)} before reading the envelope payload.

\item \textbf{Cursor atomicity}: the producer and consumer cursors
of each queue are 32-bit values updated with \texttt{atomicAdd} and
\texttt{atomicCAS}, which are sequentially consistent at
\texttt{system} scope on Hopper.
\end{enumerate}
...
\texttt{crates/ringkernel-cuda/src/k2k.cu}, emits the fences at the
right places; we sketch the correspondence in
\Cref{sec:implementation:k2k}.
...
model: the host's \texttt{cudaMemcpyAsync} establishes an
acquire-release pair on the device side, so the SM's view of
\(\mathit{live}\) at the moment of the copy is exactly the bytes
the host receives into \(\mathit{snap}\).  The formal model treats
this as instantaneous; the runtime's actual copy cost is bounded
by HBM bandwidth (\(\sim\)3~TB/s on H100, so a 4-MiB snapshot copies
in \(\sim\)1.4~$\mu$s).
