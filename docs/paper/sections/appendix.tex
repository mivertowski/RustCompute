...
\state{InvalidTarget} (the precondition of \Cref{rule:k2k-send}
implicitly requires the destination to be \Active{}).
...
\texttt{GET /api/v1/admin/actor/:id/snapshot} for as long as the
operator has not issued \textsc{Release}.
...
The six rules together produce the lifecycle state machine:
...
\Active{}
\begin{cases}
\xrightarrow{\textsc{Process}} \Active{} & \text{(self-loop)} \\
\xrightarrow{\textsc{Quiesce}} \Draining{}
  \xrightarrow{\textsc{Terminate}} \Terminated{}
  \xrightarrow{\textsc{Release}} \Dormant{} \\
\xrightarrow{\textsc{Fail}} \Failed{}
  \xrightarrow{\textsc{Restart}} \Initializing{}
\end{cases}
\]
